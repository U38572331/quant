...
OUTPUT_FILE = "nq_strategy_chart_5m.html"
...
# Resample to 5M for calculation AND plotting
...
# Calculate ATR on 5m or 1m? 
# Usually ATR is timeframe dependent. 
# User asked to "Execute on 5m K-lines". We should likely use ATR(10) of 5m data?
# Or keep using 1m ATR as per backtest?
# To stay true to "Execute ON 5m", I will calculate ATR on 5m bars.
...
# --- Simulation Logic (Adapted to run purely on 5m DF) ---
trades = []
dates = df.index.date
...
for trade_date in unique_dates:
    ...
    # Check bounds
    if session_start_dt not in day_data.index and session_start_dt < day_data.index[0]:
        # Handle case where 9:30 bar might be missing or resampling aligned differently
        pass
    
    # ORB High/Low from 5m bars
    # Which 5m bars? [09:30, 09:35, 09:40] for 15m ORB.
    # Included bars: < 09:45.
    ...
    # Trading Data (Post ORB)
    trading_data = day_data.loc[orb_end_dt : session_end_dt]
    if trading_data.empty: continue
    
    # Find Entries
    long_mask = trading_data['close'] > orb_high
    long_entries = trading_data[long_mask].index.tolist()
    
    short_mask = trading_data['close'] < orb_low
    short_entries = trading_data[short_mask].index.tolist()
    ...
    while trades_today < 2 and current_idx < len(possible_entries):
        ...
        # Exit Check (Using 5m bars - Coarser granularity)
...
fig.update_layout(
    ...
    title="NQ Strategy on 5-Minute Chart (Last 30 Days)",
...
print(f"5m Chart saved to {out_path}")
