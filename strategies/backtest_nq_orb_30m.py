...
df_rth = df[(df.index.time >= time(9, 30)) & (df.index.time <= time(16, 0))].copy()
...
# Merge ORB levels back to RTH data
...
if trades_df.empty:
    ...
    plt.grid(True)
    plt.legend()
    plt.savefig(r'C:\Users\user\.gemini\antigravity\scratch\nq_orb_equity_30m.png')
    print("\nEquity curve saved to nq_orb_equity_30m.png")

    trades_df.to_csv(r'C:\Users\user\.gemini\antigravity\scratch\nq_orb_trades_30m.csv', index=False)
    print("Trades log saved to nq_orb_trades_30m.csv")
